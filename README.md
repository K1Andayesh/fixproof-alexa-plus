...
1. Read the impact card, choose an exact reference model, then start a fictional drying case. The three Bosch models expose ten paths. Electrolux ESF8735ROX exposes eight paths: drying, food remnants, detergent residue, removable streaks, wash noise, cutlery rust, interior odour and door-related starting; the selector disables its unmapped paths.
...
| Handover can be checked independently | The separate MIT-licensed [FixProof Evidence Verifier](https://github.com/K1Andayesh/fixproof-evidence-verifier) validates a live-MCP-exported fictional bundle with only the Python standard library; its tests show changed-field detection and malformed-schema rejection, even when a supplied digest matches |
...
| Developer feedback | [Three reproducible hackathon friction entries](FRICTION_LOG.md), including the implementation path, MCP verification, and rules-link continuity |
...
- Four exact reference models: Bosch SMS6HAI02A/01, SMS6HCI01A/38 and SMS6HCI02A/72, plus Electrolux ESF8735ROX. The three Bosch models have ten paths and thirty-one shared check definitions, independently mapped to their official manuals. The Electrolux model has nineteen checks across eight paths mapped to its own manual, including brand-specific AirDry, dispenser, interior-care, noise and cutlery guidance. Unsupported model-path combinations stop without a suggested check. The [Electrolux page map](validation/ELECTROLUX_EIGHT_PATH_SCOPE.md) records the source boundary.
...
- Public judge-callable TypeScript MCP SDK 2.0.0 deployment over Streamable HTTP, with D1-backed state across connections, an enforced fictional-only input boundary, and machine-facing instructions that distinguish the Bosch ten-path catalog from the Electrolux eight-path subset.
