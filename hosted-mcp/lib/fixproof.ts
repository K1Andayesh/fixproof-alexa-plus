...
export type Step = { workflow: "drying" | "food" | "detergent" | "streaks" | "noise" | "rust" | "clouding" | "odour"; title: string; text: string; pages: number[] };
...
const definitions: Record<string, Omit<Step, "pages">> = {
  ...
  odour_wipe_interior: { workflow: "odour", title: "Remove coarse interior soiling", text: "Remove coarse soiling from the appliance interior with a damp cloth." },
  odour_clean_filters: { workflow: "odour", title: "Clean the filters", text: "Check the filters for residue and clean them under running water as described in the manual." },
  odour_machine_care: { workflow: "odour", title: "Run Machine Care", text: "Run Machine Care without tableware. Use only products designed for dishwashers and follow the product packaging safety instructions." },
};
...
export async function askFixProof(input: { request_id: string; case_id: string; revision: number; user_message: string }) {
  ...
  if (!caseRecord.workflow) {
    caseRecord.latest_event = { kind: "clarification", text: "Is the issue wet tableware, food remnants, detergent residue, removable streaks, knocking or rattling during the wash, rust spots on cutlery, irreversible glass clouding that does not wipe off, or an unpleasant odour inside the appliance?" };
    return view(await saveUpdated(caseRecord, requestId, input.revision));
  }
  const steps = stepsFor(caseRecord.model);
