...
The server exposes `start_case`, `read_case`, `ask_fixproof`, `record_outcome`, and `prepare_handover`. It also exposes the handover App as a `text/html;profile=mcp-app` resource. The App has no external scripts, styles, images, or network calls; it renders only the structured handover returned by the tool. Three exact Bosch models support ten bounded paths each; Electrolux ESF8735ROX supports eight separately mapped paths. Each selected check returns the official manual identity, exact page URL, verification date, and locally verified source hash.
