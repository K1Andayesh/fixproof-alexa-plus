...
| --- | ---: | ---: | ---: | ---: |
| Ordinary transcript | 27 | 243 / 270 | 90.0% | 5 |
| FixProof structured handover | 27 | 254 / 270 | 94.1% | 0 |

In this controlled synthetic run, the structured handover improved exact fact retrieval by 11 of 270 scored fields and eliminated the five critical errors made from the transcript. Two transcript errors lost or changed an explicit safety report; three incorrectly classified a deferred or pending check as performed.

The advantage was not universal. The handover scored higher for the drying, detergent, safety-stop, rust, clouding and odour cases, lower for the food-remnant and wash-noise cases, and tied for door-related starting. The aggregate result supports the evidence structure's value while showing that format alone does not guarantee better retrieval on every case.
...
- Nine fictional cases cover performed, deferred, skipped, pending and safety-stop evidence across all nine implemented issue paths.
...
The script requires a local Ollama service and the three named reader models. Model output can vary with runtime or model versions. This is one pre-declared synthetic suite, not an independent evaluation or statistical study.
