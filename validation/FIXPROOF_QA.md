...
| Ninth door-related-starting path | Local application logic and manual review verified three bounded checks across all exact-model catalogs; production Chrome verification is recorded after deployment |
...
- An official MCP client called the public `https://fixproof-mcp.keyvan-andayesh.chatgpt.site/api/mcp` deployment without local services, negotiated protocol `2026-07-28`, received server version 0.8.0 and instructions naming all eight verified paths, discovered all five tools, selected third-model pages 45 and 24, retained an explicit observation across a fresh connection, excluded the recorded check, and preserved a later safety stop. The Python client independently recomputed the TypeScript handover fingerprint, matched all 64 hexadecimal digits, then changed one evidence field and obtained a different digest. Additional public cases selected wash noise on page 49, cutlery rust on page 46, irreversible glass clouding on page 46 and second-model unpleasant interior odour on page 38. The production landing page reports three exact models, eight source-backed paths and a fictional-only data policy. See [HOSTED_MCP.json](HOSTED_MCP.json).
