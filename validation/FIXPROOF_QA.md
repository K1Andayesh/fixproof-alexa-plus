...
- Twenty-nine Python workflow and MCP regression tests pass, including separate drying, food-remnant, detergent-residue, removable-streak, wash-noise, cutlery-rust and irreversible-glass-clouding selection, cross-path pending-check protection, retries, origin checks, explicit evidence recording, SQLite persistence, stale writes, model failure, invalid model output, and cross-client continuity.
- Fifteen public-interface logic tests pass against the actual hosted JavaScript, including all seven source-backed journeys, cross-path protection and the live MCP proof link.
- An official MCP client called the public `https://fixproof-mcp.keyvan-andayesh.chatgpt.site/api/mcp` deployment without local services, negotiated protocol `2026-07-28`, received server version 0.7.0 and instructions naming all seven verified paths, discovered all five tools, selected third-model pages 45 and 24, retained an explicit observation across a fresh connection, excluded the recorded check, and preserved a later safety stop. The Python client independently recomputed the TypeScript handover fingerprint, matched all 64 hexadecimal digits, then changed one evidence field and obtained a different digest. Additional public cases selected the wash-noise path on page 49, cutlery-rust on page 46 and irreversible glass clouding on page 46. The production landing page reports three exact models, seven source-backed paths and a fictional-only data policy. See [HOSTED_MCP.json](HOSTED_MCP.json).
- A separate Apps-capable client connected to the running Streamable HTTP endpoint at `/mcp`, negotiated protocol `2026-07-28` and the official MCP Apps extension, discovered all five tools, read the handover's self-contained `ui://` resource, exercised all seven source-backed paths and three exact models with real local AI, recorded an outcome, and verified that the handover retained its observation across new client connections. Browser QA rendered the fingerprint and its no-authorship boundary in the actual MCP App. A request with an untrusted browser Origin was rejected with HTTP 403.
- Sixteen local-model scenarios pass: drying, food remnants, detergent residue, removable streaks, wash noise, cutlery rust, irreversible glass clouding, unclear symptom, plastic, inside-wall condensation, unsupported error/drainage issue, hazard, explicit no-hazard language, a technical phrase, already-recorded check and exhausted catalog. Full results and local latency/token observations: [LOCAL_AI_EVAL.json](LOCAL_AI_EVAL.json).
- A controlled same-fact retrieval evaluation now covers all seven source-backed paths. Across 28 blind-format reads by two local model families, structured handovers recovered 133/140 scored fields with zero critical errors; ordinary transcripts recovered 127/140 with two critical errors. These are synthetic machine-reader results, not evidence from people or completed repairs. Full inputs, outputs and scores: [HANDOVER_RETRIEVAL_EVAL.json](HANDOVER_RETRIEVAL_EVAL.json).
