...
- Twenty-five Python workflow and MCP regression tests pass, including separate drying, food-remnant and detergent-residue selection, cross-path pending-check protection, retries, origin checks, explicit evidence recording, SQLite persistence, stale writes, model failure, invalid model output, and cross-client continuity.
- Ten public-interface logic tests pass against the actual hosted JavaScript, including the food-remnant and detergent-residue journeys and cross-path protection.
- A separate client connected to the running Streamable HTTP endpoint at `/mcp`, negotiated protocol `2026-07-28`, discovered all five tools, exercised all three source-backed paths with real local AI, recorded an outcome, and verified that the handover retained its observation across new client connections. A request with an untrusted browser Origin was rejected with HTTP 403.
- Twelve local-model scenarios pass: drying, food remnants, detergent residue, unclear symptom, plastic, inside-wall condensation, unsupported error/drainage issue, hazard, explicit no-hazard language, a technical phrase, already-recorded check and exhausted catalog. Full results and local latency/token observations: [LOCAL_AI_EVAL.json](LOCAL_AI_EVAL.json).
...
- The prior hosted build passed an actual Chrome desktop journey for drying, reload/resume, handover content and judge-mode boundaries. The three-path production release receives a separate browser verification recorded in the reliability review.
