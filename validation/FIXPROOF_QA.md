...
- A controlled same-fact retrieval evaluation now covers all eight source-backed paths. Across 48 blind-format reads by three local model families, structured handovers recovered 230/240 scored fields with zero critical errors; ordinary transcripts recovered 215/240 with five critical errors. The handover did not win every scenario, so the result supports an aggregate retrieval advantage rather than a universal one. These are synthetic machine-reader results, not evidence from people or completed repairs. Full inputs, outputs and scores: [HANDOVER_RETRIEVAL_EVAL.json](HANDOVER_RETRIEVAL_EVAL.json).
