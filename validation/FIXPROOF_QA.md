...
| Portable browser evidence | Production Site version 30 recorded a fictional performed outcome, rendered a full 64-digit SHA-256 fingerprint and the no-authorship boundary in the handover preview, and exposed a `Download evidence JSON` control beside the Markdown export |
...
The Markdown download link and the version-30 structured JSON download control were clicked in production Chrome. Their actual native download locations were not verified: Chrome's internal downloads page was blocked by browser policy, and no workaround was attempted. The Markdown export endpoint was independently checked for 200 status, content type, attachment filename, page links and factual content; its returned bytes are saved as [EXAMPLE_HANDOVER.md](EXAMPLE_HANDOVER.md). The deployed `app.js` bytes matched the reviewed local release exactly by SHA-256 and contained both the `fixproof-handover-1` schema and `fixproof-evidence.json` filename. The automated interface test independently recomputed the JSON digest, matched all 64 hexadecimal digits, then changed the issue field and obtained a different digest. These checks verify the export implementation and browser control, not the browser's final disk write.
