...
STEP_DEFINITIONS = {
    ...
    'electrolux_dispenser_lid': {'workflow': 'detergent', 'title': 'Clear the detergent dispenser lid', 'text': 'Check that items in the baskets do not prevent the detergent dispenser lid from opening.', 'pages': [21]},
    'electrolux_dispenser_spray': {'workflow': 'detergent', 'title': 'Check spray-arm clearance for the dispenser', 'text': 'Check that the spray arm is not blocked or clogged so water can reach the detergent dispenser.', 'pages': [21]},
    'electrolux_detergent_dose': {'workflow': 'streaks', 'title': 'Check the detergent quantity', 'text': 'If the detergent quantity is too high, use the correct amount stated on its packaging.', 'pages': [20, 15]},
...
CATALOGS = {
    'SMS6HAI02A/01': {
        ...
        'source': SOURCE,
        'step_pages': {key: value['pages'] for key, value in STEP_DEFINITIONS.items() if not key.startswith('electrolux_')},
