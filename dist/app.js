...
function classify(message){const m=message.toLowerCase();if(/smoke|burn|electrical|electric shock|sparks?|flood|leak|exposed wir|open.*(?:panel|casing)|bypass.*(?:lock|switch)/.test(m))return'hazard';if(/\be\d{2}\b|error|drain|pump|won.t start/.test(m))return'unsupported';if(/plastic/.test(m)&&/wet|dry/.test(m))return'plastic';if(/inside|inner wall|tub/.test(m)&&/wet|drop|moist|condens/.test(m))return'interior';if(!/wet|dry|next|check|dish|glass|plate|programme|program|rinse|water/.test(m))return'unclear';return'drying';}
